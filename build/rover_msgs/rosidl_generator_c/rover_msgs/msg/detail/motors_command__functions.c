...
rover_msgs__msg__MotorsCommand__create(void)
{
  ...
  rover_msgs__msg__MotorsCommand__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rover_msgs__msg__MotorsCommand * data =
      (rover_msgs__msg__MotorsCommand *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rover_msgs__msg__MotorsCommand__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rover_msgs__msg__MotorsCommand__fini(&output->data[i]);
        }
        return false;
      }
    }
